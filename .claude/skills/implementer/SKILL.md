...
### `command`
- Path: `commands/<name>.md` (plugin-users) OR `.claude/commands/<name>.md` (sloth-internal)
...
### `skill`
- Path: `skills/<name>/SKILL.md` (plugin-users) OR `.claude/skills/<name>/SKILL.md` (sloth-internal)
...
- Templates are reference content used by skills (e.g. CLAUDE.md template, ADR template). Keep them minimal and parameterized with `<<placeholders>>`.
