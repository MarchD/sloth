---
name: syncer
description: Audits sloth's meta-files for drift and proposes updates. Checks CLAUDE.md status, PROGRESS_TRACKER freshness, README↔commands listing consistency, and version match between plugin.json and marketplace.json.
---
...
- List `commands/` (user-facing plugin commands only — NOT `.claude/commands/`)
- Read README.md
...
### 3. CLAUDE.md status section
- Read `CLAUDE.md` "What this is" / "Status & next milestone" section
- Read `PROGRESS_TRACKER.md` current milestone
- Verify CLAUDE.md doesn't reference an outdated milestone or version
...
### 5. Skills listing
- List `skills/` (plugin) and `.claude/skills/` (internal)
- Verify each is documented somewhere — README for plugin skills, CLAUDE.md for internal skills
